...
namespace Open3270.TN3270
{
    ...
    internal class Ansi:IDisposable
    {
		...
		private const byte UP = 4;    /* cursor up */
		private const byte E2 = 5;    /* second level of ESC processing */
		private const byte rS = 6;    /* reset */
		private const byte IC = 7;    /* insert chars */
		private const byte DN = 8;    /* cursor down */
		private const byte RT = 9;    /* cursor right */
		private const byte LT = 10;   /* cursor left */
		private const byte CM = 11;   /* cursor motion */
		private const byte ED = 12;   /* erase in display */
		private const byte EL = 13;   /* erase in line */
		private const byte IL = 14;   /* insert lines */
		private const byte DL = 15;   /* delete lines */
		private const byte DC = 16;   /* delete characters */
		private const byte SG = 17;   /* set graphic rendition */
		private const byte BL = 18;   /* ring bell */
		private const byte NP = 19;   /* new page */
		private const byte BS = 20;   /* backspace */
		private const byte CR = 21;   /* carriage return */
		private const byte LF = 22;   /* line feed */
		private const byte HT = 23;   /* horizontal tab */
		private const byte E1 = 24;   /* first level of ESC processing */
		private const byte Xx = 25;   /* undefined control character (nop) */
		private const byte Pc = 26;   /* printing character */
		private const byte Sc = 27;   /* semicolon (after ESC [) */
		private const byte Dg = 28;   /* digit (after ESC [ or ESC [ ?) */
		private const byte RI = 29;   /* reverse index */
		private const byte DA = 30;   /* send device attributes */
		private const byte SM = 31;   /* set mode */
		private const byte RM = 32;   /* reset mode */
		private const byte DO = 33;   /* return terminal ID (obsolete) */
		private const byte SR = 34;   /* device status report */
		private const byte CS = 35;   /* character set designate */
		private const byte E3 = 36;   /* third level of ESC processing */
		private const byte DS = 37;   /* DEC private set */
		private const byte DR = 38;   /* DEC private reset */
		private const byte DV = 39;   /* DEC private save */
		private const byte DT = 40;   /* DEC private restore */
		private const byte SS = 41;   /* set scrolling region */
		private const byte TM = 42;   /* text mode (ESC ]) */
		private const byte T2 = 43;   /* semicolon (after ESC ]) */
		private const byte TX = 44;   /* text parameter (after ESC ] n ;) */
		private const byte TB = 45;   /* text parameter done (ESC ] n ; xxx BEL) */
		private const byte TS = 46;   /* tab set */
		private const byte TC = 47;   /* tab clear */
		private const byte C2 = 48;   /* character set designate (finish) */
		private const byte G0 = 49;   /* select G0 character set */
		private const byte G1 = 50;   /* select G1 character set */
